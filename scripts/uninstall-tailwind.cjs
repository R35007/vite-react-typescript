...
function removeTailwind() {
  try {
    fs.unlinkSync(rootDir + '/postcss.config.cjs');
    console.log('remove postcss.config.cjs');
    fs.unlinkSync(rootDir + '/tailwind.config.cjs');
    console.log('remove tailwind.config.cjs');
  } catch (e) {
    if (e.message.includes('no such file or directory')) console.log('postcss.config.cjs/tailwind.config.cjs has already been removed.');
  }
